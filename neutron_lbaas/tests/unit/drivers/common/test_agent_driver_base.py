...
class TestLoadBalancerManager(test_agent_scheduler.
    ...
    def test_start_periodic_lbaas_agent_status_check(self):
        with mock.patch(
            'neutron_lbaas.drivers.common.agent_driver_base.'
            'LoadBalancerManager.add_agent_status_check'
        ) as mock_agent_status_check:
            # allow_automatic_lbaas_agent_failover is disabled
            cfg.CONF.set_override('allow_automatic_lbaas_agent_failover',
                                  False)
            self.load_balancer.start_periodic_lbaas_agent_status_check()
            self.assertFalse(mock_agent_status_check.called)
            # allow_automatic_lbaas_agent_failover is enabled
            cfg.CONF.set_override('allow_automatic_lbaas_agent_failover',
                                  True)
            self.load_balancer.start_periodic_lbaas_agent_status_check()
            self.assertTrue(mock_agent_status_check.called)
            mock_agent_status_check.assert_called_once_with(
